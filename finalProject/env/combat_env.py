...
class CombatEnv(SocketEnv):
    def __init__(self,
                 ip: str, port: int,
                 mock_policy_fcn: Callable[[Any, Any], PyPolicy],
        ...
        self._mock_policy = mock_policy_fcn(
            time_step_spec(
                BoundedArraySpec((self._obs_info[0],), np.float32,
                                 minimum=self._obs_info[1], maximum=self._obs_info[2], name='observation'),
                ArraySpec(shape=(), dtype=np.float32, name='reward')
            ),
            BoundedArraySpec((self._action_info[0],), np.float32,
                             minimum=self._action_info[1], maximum=self._action_info[2], name='action')
        )
    ...
    def _gen_concat_obs(self, obs, obs_mock):
        def _cal_damage(rx, ry, rz, pitch, yaw):
            ...
            vec_head = np.array([math.cos(yaw), math.sin(yaw), math.sin(pitch)])
    ...
    def _gen_actions(self, all_observations: List[np.ndarray]) -> List[np.ndarray]:
        return [self._mock_policy.action(transition(observation=all_observations[0], reward=0.)).action]