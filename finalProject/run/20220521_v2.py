...
sac_prefix = './save/20220521_v2/sac'
dqn_prefix = './save/20220521_v2/dqn'
tot = 0
...
def get_reward(obs: np.ndarray, prev_obs: np.ndarray) -> Tuple[float, bool]:
    ...
    speed = norm(linear_speed, ord=2)
    opp_speed = norm(opp_linear_speed, ord=2)
    d_pos = pos-opp_pos
    ...
    r_pos = (800-norm(pos-opp_pos, ord=2)) * 0.1
    r_damage = (damage_cause-damage_suffer)*3

    d_pos=d_pos/norm(d_pos)
    angle=angle/norm(angle)
    opp_angle=opp_angle/norm(opp_angle)

    angle_1 = np.arccos(np.dot(d_pos,opp_angle)) # 脱离角
    angle_2 = np.arccos(np.dot(d_pos,angle))  # 偏离角

    r_angle = (2-2*(angle_1+angle_2)/np.pi)*10
    tot += 1
    if tot % 1000 == 0:
        logger.info("tot:{}, speed:{}, opp_speed:{}, r_v_self:{}, r_v:{}, r_pos:{}, r_damage:{}, r_angle:{}, damage_cause:{}, damage_suffer:{}".format(
            tot, speed, opp_speed, r_v_self, r_v, r_pos, r_damage, r_angle, damage_cause, damage_suffer))
    return r_v_self+r_v+r_pos+r_damage+r_angle, False
