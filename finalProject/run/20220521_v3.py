...
sac_prefix = './save/20220521_v3/sac'
dqn_prefix = './save/20220521_v3/dqn'
...
def get_reward(obs: np.ndarray, prev_obs: np.ndarray) -> Tuple[float, bool]:
    ...
    speed = norm(linear_speed)
    opp_speed = norm(opp_linear_speed)
    ...
    r_v_self = (speed/300-0.5)*10 if speed < 150 else 0
    r_v = (speed-opp_speed)/10
    r_pos = (800-norm(pos-opp_pos)) * 0.07
    r_damage = (damage_cause-damage_suffer)*3

    if tot%100==0:
        logger.info("tot:{}, speed:{}, r_v_self:{}, r_v:{}, r_pos:{}, r_damage:{} damage_cause:{}, damage_suffer:{}".format(tot, speed, r_v_self, r_v, r_pos, r_damage, damage_cause, damage_suffer))
