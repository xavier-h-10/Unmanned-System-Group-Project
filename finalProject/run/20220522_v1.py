...
sac_prefix = './save/20220522_v1/sac'
dqn_prefix = './save/20220522_v1/dqn'
...
def get_reward(obs: np.ndarray, prev_obs: np.ndarray) -> Tuple[float, bool]:
    ...
    r_v_self = (speed/300-0.5)*100 if speed < 150 else 0
    r_v = (speed-opp_speed)*0.5
    r_pos = (750-dis) * 0.1
    r_damage = (damage_cause-damage_suffer)*30
    r_delta_pos = (prev_dis-dis)*0.1

    if tot % 100 == 0:
        logger.info("tot:{}, speed:{}, dis:{}, prev_dis:{}, r_delta_pos:{}, r_v_self:{}, r_v:{}, r_pos:{}, r_damage:{} damage_cause:{}, damage_suffer:{}".format(
            tot, speed, dis, prev_dis, r_delta_pos, r_v_self, r_v, r_pos, r_damage, damage_cause, damage_suffer))
