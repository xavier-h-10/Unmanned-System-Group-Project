...
def get_reward(obs: np.ndarray, prev_obs: np.ndarray) -> Tuple[float, bool]:
    ...
    r_v_self = (speed/1200-0.5)*1000 if speed < 600 else 0
    r_v = (speed-opp_speed)*1
   # r_pos = (750-dis) * 0.1

    r_pos = 0.02*(2000-dis) if dis > 2000 else 2000-dis

    r_damage = (damage_cause-damage_suffer)*30
    r_delta_pos = (prev_dis-dis)*6

    r = r_delta_pos+r_v_self+r_v+r_pos+r_damage

    if (tot // 300) % 10 == 0:
        logger.info("tot:{}, reward:{}, pos:{}, opp_pos:{}, dis:{}, prev_dis:{}, speed:{}, opp_speed:{}, r_delta_pos:{}, r_v_self:{}, r_v:{}, r_pos:{}, r_damage:{} damage_cause:{}, damage_suffer:{}".format(
            tot, r, pos, opp_pos, dis, prev_dis, speed, opp_speed, r_delta_pos, r_v_self, r_v, r_pos, r_damage, damage_cause, damage_suffer))
